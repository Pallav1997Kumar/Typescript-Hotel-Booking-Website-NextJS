...
function EventsMeetingsEachRoom(props: IEventMeetingRoomProps) {
    return (
        <ErrorBoundary>
            <EventsMeetingsEachRoomFunctionalComponent 
                currentMeetingEvent={props.currentMeetingEvent} 
            />
        </ErrorBoundary>
    );
}
...
function EventsMeetingsEachRoomFunctionalComponent(props: IEventMeetingRoomProps) {
    ...
    const unsortedMeetingEventAreaSeatingCapacityInfo: MeetingEventAreaSeatingCapacity[] = 
        currentMeetingEvent.meetingEventAreaSeatingCapacityInfo;

    const onlyMeetingEventsSeatingInfoWhereSeatingPresent: MeetingEventAreaSeatingCapacity[] = 
        unsortedMeetingEventAreaSeatingCapacityInfo.filter(function(
            eachSeatingArrangement: MeetingEventAreaSeatingCapacity
        ){
            return (eachSeatingArrangement.meetingEventAreaSeatingCapacity != 'N/A');
        });

    const sortedMeetingEventAreaSeatingCapacityInfo: MeetingEventAreaSeatingCapacity[] = 
        onlyMeetingEventsSeatingInfoWhereSeatingPresent.toSorted((a,b) => Number(b.meetingEventAreaSeatingCapacity) - Number(a.meetingEventAreaSeatingCapacity));
    
    const maximumSeatingCapacity: string | number = 
            sortedMeetingEventAreaSeatingCapacityInfo[0].meetingEventAreaSeatingCapacity;

    return (
        <div className="flex flex-row m-[5%_3%] border border-blue-500 p-[1%] bg-[rgb(250,239,226)]">
            <div className="w-2/5">
                <Image 
                    src={currentMeetingEvent.meetingEventAreaImage} 
                    alt="current-event-meeting" 
                    width={500} 
                    height={300} 
                />
            </div>
                ...
                <div className="mt-[5%] flex items-center justify-center">
                    <Link href={`/meetings-events/${meetingEventAreaPath}`}> 
                        <Button variant="contained">
                            EXPLORE EVENT AND MEETING AREA
                        </Button>
                    </Link>
                </div>
            </div>
