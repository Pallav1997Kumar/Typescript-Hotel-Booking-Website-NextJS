...
function UserCurrentEventMeetingBookingPageFunctionalComponent(){
    ...
    async function fetchEventMeetingBookingDb(loginUserId: string) {
        try {
            const response: Response = await fetch(`/api/view-current-booking/meeting-events/${loginUserId}`);
            ...
            <div className="m-6 bg-[#f0f8ff] p-4">
                <p className="text-xl font-bold tracking-wider text-blueviolet">
                    ...
                    <Link href="/profile-home-page"> 
                        <span className="text-xl text-indigo-600 hover:text-[#8b008b] hover:underline"> PROFILE PAGE </span>
                    </Link>
                    <span className="px-3">{'>>'}</span>  
                    <Link href={`/profile-home-page/view-current-bookings/${loginUserId}`}> 
                        <span className="text-xl text-indigo-600 hover:text-[#8b008b] hover:underline"> VIEW CURRENT OR UPCOMING BOOKINGS </span>
                    </Link>
                    <span className="px-3">{'>>'}</span> 
                    <Link href={`/profile-home-page/view-current-bookings/events-meeting-room/${loginUserId}`}> 
                        <span className="text-xl text-indigo-600 hover:text-[#8b008b] hover:underline"> EVENT/ MEETING ROOMS BOOKING </span>
                    </Link>
                </p>
            </div>
                ...
                {(!loadingBookingDetails && eventMeetingBooking !== null && eventMeetingBooking.length == 0) &&
                    <div className="bg-blue-200 p-6 my-4 flex items-center justify-center">
                        <p className="text-red-700 font-bold text-2xl font-sans">
                            You do not have any Current or Future Upcoming Events/ Meeting Room Bookings
                        </p>
                    </div>
                }
...
export default UserCurrentEventMeetingBookingPage;