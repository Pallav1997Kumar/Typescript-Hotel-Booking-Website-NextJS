...
async function POST(request: NextRequest): Promise<NextResponse> {
    try{
        ...
        const diningBookingAmount: number = transactionInfo.deductionAmount;

        const hotelUser: IHotelCustomersUser | null = 
            await HotelCustomersUsers.findById(customerId);

        if(hotelUser){
            const transactionAmount: number = diningBookingAmount; 
            const transactionType: string = 'Money Deducted From Account';
            const transactionDescription: string = 'Money Deducted From Account By Dining Booking';
            const transactionDateTime: Date = new Date();
            const beforeAccountBalance: number = hotelUser.accountBalance;
            const updatedAccountBalance: number = Number(beforeAccountBalance) - Number(diningBookingAmount);
            ...
            return NextResponse.json(
                { 
                    message: SUCCESSFUL_AMOUNT_DEDUCT_DINING_TRANSACTION, 
                    transactionId 
                },
                { status: 200 }
            );
        ...
        console.log('src/app/api/account-balance-user/add-transaction-amount-deduct-account/dining-transaction/route')
        console.log(error);
        return NextResponse.json(
