...
async function DELETE(request: NextRequest, context: IContextTransactionId){
    try {
        ...
        const transactionInformation: ISingleDateBookingInfo[] = await SingleDateBookingInfo.find({
            transactionId: transactionId
        });

        if(transactionInformation && transactionInformation.length > 0){
            await SingleDateBookingInfo.deleteMany({
                transactionId: transactionId
            });
        ...
        console.log('src/app/api/delete-booking-txnid/meeting-events/single-date/[transaction]/route.js');
        console.log(error);
        return NextResponse.json(
