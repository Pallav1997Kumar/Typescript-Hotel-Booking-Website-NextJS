...
import { INonContinousMultipleDatesBookingInfo } from '@/interface/Event Meeting Interface/eventMeetingDbModelInterface';
...
async function DELETE(request: NextRequest, context: IContextTransactionId){
    try {
        ...
        const transactionInformation: INonContinousMultipleDatesBookingInfo[] = 
            await NonContinousMultipleDatesBookingInfo.find({
                transactionId: transactionId
            });

        if(transactionInformation && transactionInformation.length > 0){
            await NonContinousMultipleDatesBookingInfo.deleteMany({
                transactionId: transactionId
            });
        ...
        console.log('src/app/api/delete-booking-txnid/meeting-events/multiple-dates-non-continous/[transaction]/route.js');
        console.log(error);
        return NextResponse.json(
